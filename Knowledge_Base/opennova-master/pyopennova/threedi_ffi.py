...
# Compatibility aliases for the shared DCC import code, which was originally
# written against direct 3DI3 binding names.
THREEDI_MESH_INVALID = 0
THREEDI_MESH_BASIC = 1
THREEDI_MESH_SKINNED = 2

THREEDI_MODEL_MESH_INVALID = THREEDI_IR_MESH_INVALID
THREEDI_MODEL_MESH_BASIC = THREEDI_IR_MESH_BASIC
THREEDI_MODEL_MESH_STATIC = THREEDI_IR_MESH_STATIC
THREEDI_MODEL_MESH_SKINNED = THREEDI_IR_MESH_SKINNED

THREEDI_TEX_SLOT_DIFFUSE = THREEDI_IR_TEX_SLOT_DIFFUSE
THREEDI_TEX_SLOT_DETAIL = THREEDI_IR_TEX_SLOT_DETAIL
THREEDI_TEX_SLOT_NORMAL = THREEDI_IR_TEX_SLOT_NORMAL
THREEDI_TEX_SLOT_NORMAL_B = 4
THREEDI_MAX_MATERIAL_TEXTURES = 8
...
THREEDI_MATERIAL_FLAG_EMISSIVE = THREEDI_IR_MATERIAL_FLAG_EMISSIVE
THREEDI_EMISSIVE_FULL = 2


def read_model(path):
    """Read any supported .3di into the current normalized IR structure."""
    return read_model_ir(str(path))


def read_model_3di3(path):
    """Compatibility alias for reference code that asked for direct 3DI3."""
    return read_model(path)


def free_model_3di3(model) -> None:
    """Free a model returned by :func:`read_model`."""
    free_model_ir(model)
