...
// with init 0xFFFFFFFF. The table below is byte-exact from dword_7FD658
...
// Lifted from jodemo .rdata @ 0x7FD658. Do not regenerate; preserving the
// exact bytes keeps wire-level CRC output byte-identical to the original.
...
constexpr uint32_t crc32_napi_update(uint32_t crc, uint8_t byte) {
	return CRC32_TABLE[((crc >> 24) ^ byte) & 0xFF] ^ (crc << 8);
}
...
inline uint32_t crc32_napi_update(uint32_t crc, const uint8_t *data, size_t len) {
	for (size_t i = 0; i < len; ++i) {
		crc = crc32_napi_update(crc, data[i]);
	}
	return crc;
}
