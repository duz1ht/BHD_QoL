...
PredictionSnapshot GetPredictionSnapshot() {
    ...
    // Each counter is atomic. A packet arriving between these reads is safely
    // observed on the following render frame.
    snapshot.totalX = InterlockedCompareExchange(&g_totalRawX, 0, 0);
    snapshot.totalY = InterlockedCompareExchange(&g_totalRawY, 0, 0);
    snapshot.committedX = InterlockedCompareExchange(&g_committedRawX, 0, 0);
    snapshot.committedY = InterlockedCompareExchange(&g_committedRawY, 0, 0);
    return snapshot;
}
